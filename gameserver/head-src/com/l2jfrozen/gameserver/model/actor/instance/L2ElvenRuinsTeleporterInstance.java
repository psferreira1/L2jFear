 ...
 * @version 1.0
 */
public class L2ElvenRuinsTeleporterInstance extends L2NpcInstance
{
	...
	private List<L2PcInstance> _raiders = new FastList<>();
	private FastSet<L2NpcInstance> _monsters = new FastSet<>();
	...
	private boolean checkConditions(L2PcInstance player)
	{
		...
		if (party.getMemberCount() < 2) //TODO change to 5
		{
			player.sendPacket(new ExShowScreenMessage("You need party with at least 5 members to enter!", 5000));
			return false;
		}
	...
	public void showHtml(L2PcInstance player, String htm)
	{
		...
		notice.setFile("data/html/RuinsMod/");
	...
	private void teleportInside(L2PcInstance player)
	{
		if (checkConditions(player))
		{
			...
			for (L2PcInstance member: party.getPartyMembers())
			{
				...
				spawnMonsters();
				setMayEnter(false);
				ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleEnterTask(), 300000); //TODO 600000 
			}
		}
	}
	
	/**
	 * Clear Elven ruins, kick each player inside Ruins
	 * and send them info about that its time to leave :)
	 */
