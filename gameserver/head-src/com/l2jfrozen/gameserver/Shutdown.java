	...
	private void closeServer()
	{
		...
		AutoSaveManager.getInstance().stopAutoSaveManager();
					...
					try
				{
						...
						LOGGER.info("Class Balancer: Class Balances updated to database.");
					}
					catch (Throwable t)
					{
						LOGGER.warn("Error while updating class balances to database: ", t);
					}
		
					try
				{
					SkillsBalanceManager.getInstance().updateBalances();
						LOGGER.info("Skills Balancer: Skill Balances updated to database.");
					}
					catch (Throwable t)
					{
						LOGGER.warn("Error while updating skill balances to database: ", t);
					}
