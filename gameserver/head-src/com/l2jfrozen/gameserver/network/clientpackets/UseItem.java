...
public final class UseItem extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		{
			if(item.getItemId() == 2410)
			{
				activeChar.sendMessage("You Can't Use This Is Item.");
				return;
			}
		}
					...
					case L2Item.SLOT_L_FINGER:
					{
						...
						break;
			              ...
			              if (Config.ALT_DISABLE_BOW_CLASSES)
			                        ...
			                        {
			                        if(Config.DISABLE_SWORD_CLASSES.contains(activeChar.getClassId().getId()) && !(activeChar.isInOlympiadMode())){
	                        			activeChar.sendPacket(new SystemMessage(SystemMessageId.CANNOT_EQUIP_ITEM_DUE_TO_BAD_CONDITION));
	                        			return;
			                        }
			                        }
		                        ...
		                        if (Config.ALT_DISABLE_BLUNT_CLASSES)
			                        ...
			                        {
			                        if(Config.DISABLE_POLE_CLASSES.contains(activeChar.getClassId().getId()) && !(activeChar.isInOlympiadMode())){
	                        			activeChar.sendPacket(new SystemMessage(SystemMessageId.CANNOT_EQUIP_ITEM_DUE_TO_BAD_CONDITION));
	                        			return;
			                        }
			                        }
			                        ...
			                        {
			                        if(Config.DISABLE_BIGSWORD_CLASSES.contains(activeChar.getClassId().getId()) && !(activeChar.isInOlympiadMode())){
	                        			activeChar.sendPacket(new SystemMessage(SystemMessageId.CANNOT_EQUIP_ITEM_DUE_TO_BAD_CONDITION));
	                        			return;
			                        }
			                        }
