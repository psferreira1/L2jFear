...
public class AutoVoteRewardHandler1
{
    protected static final Logger LOGGER = Logger.getLogger(AutoVoteRewardHandler1.class);
    ...
    private AutoVoteRewardHandler1()
    {
        ...
        public void run()
        {
            if (_hopzone)
            {
                ...
                if (hopzone_votes != -1)
                {
					...
					CreatureSay cs1 = new CreatureSay(1, Say2.HERO_VOICE, "[Vote System]",  "Hopzone Next Reward on: " + (getHopZoneVoteCount() + Config.VOTES_FOR_REWARD) + " Votes!!");
	 				for(L2PcInstance player: L2World.getInstance().getAllPlayers())
					{
						if(player != null)
							if(player.isOnline()!=0)
                    ...
                    CreatureSay cs2 = new CreatureSay(1, Say2.HERO_VOICE, "[Vote System]",  "Website " + Config.SERVER_WEB_SITE);
	 				for(L2PcInstance player: L2World.getInstance().getAllPlayers())
					{
						if(player != null)
							if(player.isOnline()!=0)
            ...
            if (_topzone && _hopzone && Config.VOTES_SYSYEM_STEP_DELAY > 0)
            {
                try
                {
            ...
            if (_topzone)
            {
            	...
            	//        if (topzone_votes != -1)
                {
                    ...
                    //	CreatureSay cs1 = new CreatureSay(1, Say2.HERO_VOICE, "[Vote System]",  "Topzone Next Reward on: " + (getTopZoneVoteCount() + Config.VOTES_FOR_REWARD) + " Votes!!");
	 				for(L2PcInstance player: L2World.getInstance().getAllPlayers())
					{
	 					//	if(player != null)
							//			if(player.isOnline()!=0)
                    ...
                    new CreatureSay(1, Say2.HERO_VOICE, "[Vote System]",  "Website " + Config.SERVER_WEB_SITE);
                    //	for(L2PcInstance player: L2World.getInstance().getAllPlayers())
					{
						//			if(player != null)
						//				if(player.isOnline()!=0)
            ...
            if (_hopzone &&  Config.VOTES_SYSYEM_STEP_DELAY > 0)
            {
              try
                {
                    Thread.sleep(Config.VOTES_SYSYEM_STEP_DELAY / 2);
            ...
            if (_l2network &&  Config.VOTES_SYSYEM_STEP_DELAY1 > 0)
            {
              try
                {
                    Thread.sleep(Config.VOTES_SYSYEM_STEP_DELAY1 / 2);
                }
                catch (final InterruptedException e)
                {
                    if (Config.ENABLE_ALL_EXCEPTIONS)
            ...
            if (_l2network)
            {
               ...
               //     CreatureSay cs1 = new CreatureSay(1, Say2.HERO_VOICE, "[Vote System]",  "Network Next Reward on: " + (getL2NetworkVoteCount() + Config.VOTES_FOR_REWARD) + " Votes!!");
	 				for(L2PcInstance player: L2World.getInstance().getAllPlayers())
					{
	 					new CreatureSay(1, Say2.HERO_VOICE, "[Vote System]",  "Website " + Config.SERVER_WEB_SITE);
	 	//			for(L2PcInstance player: L2World.getInstance().getAllPlayers())
    ...
    protected int getHopZoneVotes()
    {
        ...
        try
        {
            ...
            final HtmlPage page = webClient.getPage(Config.VOTES_SITE_HOPZONE_URL1);
    ...
    public static AutoVoteRewardHandler1 getInstance()
    {
    ...
    {
        protected static final AutoVoteRewardHandler1 _instance = new AutoVoteRewardHandler1();
    }
}